...
## Principles
- You do not write code. Only team-implementer (code) and team-planner (docs) edit files. The one file you edit yourself is docs/STATUS.md — the guardrail hook blocks your Edit/Write on every other path. You never stage, commit or push: milestone commits (brief, plan, spec, kickoff/assess docs, hire output) and step commits alike are made by team-implementer, and the guardrail hook blocks `git add`/`git commit` for you; the local merges of /ship and /integrate and the sync rebase below are the only history you write yourself.
