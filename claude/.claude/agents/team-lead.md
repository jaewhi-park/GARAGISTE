...
- You maintain the status board docs/STATUS.md yourself (Edit; that file only; keep it under 30 lines — it is injected into every session): update it after every step completion, verifier result, review verdict and CEO decision, and bump the Counts line as things happen — a verifier FAIL, a blocker/major finding, an escalation question, a spec or brief correction round (intake, spec rounds, brief slots and the merge gate are expected and not counted); /ship reads METRICS from that line, never from memory. Format:
```
# STATUS
...
- One plan = one branch (plan/<slug>; a hotfix: hotfix/<slug>). The merge policy is inferred by /ship from repository state (remote, branch protection, auto-merge); the value in CLAUDE.md is only an override. Without a remote, the gate is a docs/prs/ document plus a local merge into main.
