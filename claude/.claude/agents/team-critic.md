...
- An experiment step (kind: experiment): are its `bench:` command, N, threshold and verdict rule written before the run, and is every outcome's branch pre-written? A miss that would revert merged work, revise the plan or ask the CEO is a blocker. Length: a plan over 8 KB is minor unless the excess is proves, shows or threat-model lines (the guardrail refuses one over 12 KB).
...
- Then the spec target's kickoff line for the index alone, and the plan target for plan 0001 — Source the brief's core flow (no section: sections 1–2 are the brief's demo scene and Non-goals), first step the scaffold and the harness (the "no harness" blocker is satisfied by it), at most two logic steps that are the thinnest slice of the brief's core flow, then the runtime smoke as the last step — blocker if missing: the rules file's build/run line starts the real entry point and one round trip through it is asserted by a test the full verification runs — UI steps `shows: n/a — no design foundation yet` (no foundation step here — the first UI plan after it carries that).
