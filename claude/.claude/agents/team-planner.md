...
- A plan whose files hit the rules file's "## UI paths" loads the `design` skill (Read .claude/skills/design/SKILL.md): every step that adds or changes a screen carries one `shows:` line per screen with its states (`shows: <screen> — empty · loading · error · success`, a state that does not apply says why) besides its proves lines; the plan's "Try it" names the same screens. A plan whose files hit "## Risk paths" loads the `security` skill (Read .claude/skills/security/SKILL.md): section 5 carries "Threat model" lines (per entry point: who may call it · with what · what must be refused) and each "must be refused" is a negative-case `proves:` line on the step that builds the entry point — never `n/a`.
