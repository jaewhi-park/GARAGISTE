...
- Commands are one line each — `- <label>: \`<command>\`` — with nothing after the closing backtick but the ` — unverified` marker; a note about a command (what it covers, how long it takes) goes to docs/ARCHITECTURE.md. The guardrail reads the backtick span as the command, so a note after it never hides the command from the verifier.
...
## Operating profile
- (filled by /hire via set-profile.mjs: budget tier / default review lenses 2|4 / parallelism none|session|parallel / plan-size target (logic steps, default 4) / per-step verifier on|off / step-size target / iteration cap (plans per "계속", default 4) / hire date)
