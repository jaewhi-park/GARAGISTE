...
2. Run `node .claude/scripts/set-language.mjs --file CLAUDE.md --settings .claude/settings.json <code>`. It creates CLAUDE.md if missing or rewrites only its "## Language" section, and sets the `language` key of .claude/settings.json (Claude Code's own response-language setting; it takes effect at the next session). Never edit either file yourself.
