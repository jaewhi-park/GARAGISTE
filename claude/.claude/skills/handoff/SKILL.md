...
2. Have team-planner update docs/STATUS.md in the status-board format: plan and branch (or, while no plan exists yet, the spec in progress: path and round), step progress and last verifier result, open review findings, decisions waiting on the CEO, next three actions (for a stopped spec, the first is `/plan docs/specs/NNNN-<slug>.md`).
