...
4. Resume from the first "next action" (a board whose only Now line is a Brief line resumes with `/brainstorm` while it is brainstorm, draft or correction, and with `/kickoff` or `/assess <path>` once approved). If that action is a workflow command (/run etc.), do not invoke it via the Skill tool — suggest it to the CEO in a sentence.
