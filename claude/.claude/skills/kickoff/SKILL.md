...
3. Have team-implementer create the project skeleton per the confirmed docs/adr/0001-stack.md (minimal structure where build, tests and lint pass empty; tell it CLAUDE.md has no Commands section yet, so it runs the stack's standard build/test/lint commands instead) and report the exact commands it ran with their exit codes. Do not call team-verifier yet — CLAUDE.md has no "## Commands" section for it to run.
