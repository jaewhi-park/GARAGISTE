...
그 외는 팀이 한다 — 사양서를 쓰고, 계획하고, 만들고, 리뷰하고, 저위험 작업은 스스로 머지하며, 무인 상한(기본 계획 8개 또는 사양서 절 2개)까지 계획을 이어 돌고 거기서 멈춰 당신을 본다. 새 화면 · 첫 실행 · 기본값 결정은 멈춤이 아니라 당신 페이지의 한 줄이다. 커맨드를 칠 필요는 없다. 말하면 lead가 절차를 돌린다. diff는 읽지 않는다. 출시 뒤에는 `risk:high` 작업(데이터·인터페이스·보안)이 PR의 리스크 요약을 보고 말하는 당신의 머지 또는 보류를 기다린다.
...
| 기획서 뒤 "진행해" | `/kickoff`(레거시는 `/assess`): 질문 하나 — 스택, 예산, 그리고 모델을 부르는 제품이면 기준 모델 — 그다음 헌장·사양서 색인·백로그·첫 계획·채용. 상태판에 `Mode: running` | 질문 하나에 답하고, 새 세션을 열어 "계속" |
| "어디까지 됐어?" | 상태판을 git과 대조해 답한다: 써 볼 수 있는 것, 바뀐 것, 당신을 기다리는 것 | 없음. main에서 제품을 켜 본다(상태판의 Run 줄) |
| "계속" | 계획을 이어(plan → build → review → ship → 머지) 무인 상한까지 돌고 이터레이션 리뷰와 함께 멈춘다 | 리뷰를 읽고, 적힌 것을 써 보고, "써봤다 · 결과"나 "안 써봤다"로 답하고, 원하는 것을 말한다 — 이 세션이든 새 세션이든 |
...
| "출시했어" (이제 실사용자나 데이터가 있다) | 상태판의 Stage를 live로 바꾸고 커밋한다. 그때부터 risk:high 머지와 스키마·인터페이스 변경은 당신 한마디를 기다린다 | 출시할 때 한 번 말한다. 되돌아가지 않는다 |
| "멈춰" | 진행 중 단계를 마치고 상태판을 커밋하고 기다린다(`Mode: paused`) | 다음에 원하는 것을 말한다 |
| (팀이 묻는다) | 에스컬레이션 — 되돌리기 비싼 넷: 돈, 보안·사용자 데이터, 비목표 충돌, 머지된 작업 폐기; 그리고 출시 뒤의 `risk:high` 머지 | "결정 / 선택지 / 추천 / 기본값" 형식으로 답한다. `risk:high` 머지는 PR의 리스크 요약을 읽고 머지 또는 보류라고 말한다 |
| (팀이 스스로 멈춘다) | 이터레이션 리뷰: 써 볼 것 · 바뀐 것 · 당신이 할 일 · 기본값으로 정한 것 · 회고 제안 · 다음 이터레이션 | 써 본다. 그다음 "계속"(또는 "1, 3 반영하고 계속") 또는 지시 |
...
당신이 받는 질문은 이것뿐이다. 킥오프 때 스택과 예산 한 번(모델을 부르는 제품이면 기준 모델도); 돈이 드는 것(유료 서비스, 새 런타임 의존성); 보안·인증·사용자 데이터에 닿는 것; 기획서의 비목표를 넘는 요청; 머지된 작업을 버리게 하는 변경; 출시 뒤에는 `risk:high` 머지 — diff가 아니라 PR의 리스크 요약으로 정한다. 나머지는 팀이 기본값으로 정한다: 이번 이터레이션의 것은 당신의 페이지(docs/STATUS.md의 "기본값으로 정한 것")에, 되돌리기 비싼 것은 docs/DECISIONS.md에 있고, 말하면 뒤집힌다.
...
첫 실행: 레포에서 `claude` → 폴더 신뢰 확인에 동의(훅이 켜진다) → settings의 `agent` 설정으로 team-lead가 메인 에이전트다 → 아이디어를 말한다(또는 직접 쓴 문서의 경로를 준다): lead가 함께 브레인스토밍하고 기획서를 정리한 뒤 질문 한 라운드(비목표 3개, 정해 둔 기본값, critic 지적)를 묻고 승인을 청한다 → 승인이 킥오프를 시작한다: 질문 하나(스택과 예산, 모델을 부르는 제품이면 기준 모델), 그다음 팀이 헌장·스택 ADR·규칙 파일·사양서 색인·백로그·첫 계획(walking skeleton: 골격과 최대 두 단계)을 쓰고 상태판을 `Mode: running`으로 남긴다 → 새 세션을 열고(모델 배정은 세션 시작 때 로드된다) "계속".
...
### 모델·예산 — `/hire`
`/kickoff` 또는 `/assess` 가 끝나면 lead 가 거기서 답한 예산 티어와 기획서의 프로젝트 성격으로 `/hire` 를 실행한다. 사용 가능한 모델(별칭 opus/sonnet/haiku)을 확인하고 역할 × 모델 표를 이유와 함께 만들어 바로 적용한다 — 두 번 묻지 않는다. 배정표가 보이고, 한 줄 바꾸려면 말하면 된다. 예산이 바뀌면 `/hire` 를 직접 실행한다(그때는 티어를 묻는다). 원칙: 판단하는 자리(planner·critic·reviewer)에 강한 모델, verifier 는 가장 빠른 모델, implementer 는 예산에 따라. 스크립트가 `model:` 줄만 바꾸고 CLAUDE.md 에 "운영 프로필"이 남으며, 새 세션부터 반영된다. 현재 배정은 `/roster` 로 본다.

### 언어
에이전트 프롬프트는 영어다. 응답·질문·문서는 CLAUDE.md의 `## Language` 줄을 따른다. 첫 브레인스토밍 때 lead가 한 번 묻고, `/lang <code>`로 언제든 바꾼다(스크립트가 그 절만 다시 쓴다). 비어 있을 때 에이전트는 당신의 마지막 메시지 언어를 따르지만, 영어 명령 템플릿 때문에 lead가 영어로 넘어갈 수 있다. 그러면 말하거나 `/lang`을 실행한다.

## 2. 팀은 당신 없이 이렇게 돈다

- **당신 것인 문서는 기획서 하나다.** docs/BRIEF.md가 당신이 승인한 것이다. 팀은 거기서 헌장(모든 계획을 재는 헌법)을 도출하고, 기획서와 부록에 남은 당신 말로 사양서를 쓴다 — 킥오프 때는 docs/SPEC.md 색인에 기능마다 한 행(F1, F2, …), 그 기능의 차례가 오면 docs/specs/에 파일 하나. 빈 슬롯은 기본값으로 채우지 질문하지 않는다 — 이번 이터레이션의 것은 당신의 페이지에, 되돌리기 비싼 것은 docs/DECISIONS.md에. 에스컬레이션 넷만 예외다.
- **일은 이터레이션으로 돈다.** 첫 이터레이션은 walking skeleton 하나다. 그 위에 무언가 쌓이기 전에 당신이 본다. 그다음부터 lead가 백로그 항목을 순서대로, 무인 상한(기본 계획 8개 또는 사양서 절 2개, /hire가 정한다)까지 잡고, 항목마다 사양서 절에서 계획(critic이 모든 계획을 보고 APPROVE가 곧 승인) → build → review → ship → 머지. 상한에서 이터레이션 리뷰를 쓰고 멈춘다. 그 사이의 새 화면 · 첫 실행 · 기본값 결정은 당신 페이지의 한 줄이지 멈춤이 아니다. "계속"은 이 세션에서든 새 세션에서든 된다 — 보드가 모든 것을 나른다. 출시 뒤의 `risk:high` PR은 당신이 머지할 때까지 이터레이션을 멈춘다. 다음 계획이 그 위에 쌓일 수 있어서다.
- **모든 단계는 다음 단계 전에 증명된다.** 논리 단계는 쉬운 말로 쓴 `proves:` 줄 하나에서 넷을 갖고, 각각 변경 전엔 빨강, 후엔 초록인 테스트가 된다. UI 단계는 `shows:` 줄도 갖는다 — 화면과 네 가지 상태(빈/로딩/에러/성공) — 리뷰어가 보는 스크린샷으로 증명한다. 디자인 기반이 들어간 뒤부터다(walking skeleton 다음의 첫 UI 계획이 1단계로 그것을 만든다). verifier는 build 끝과 출하 전에 전체를 돌리고, 리뷰 수정 라운드는 quick 검사와 건드린 테스트만 돈다.
- **리뷰는 렌즈별이고, 렌즈는 기계적이다.** correctness와 security는 항상, performance와 maintainability는 diff가 클 때와 출시 뒤 위험이 높을 때, ux 렌즈는 diff가 UI 경로에 닿고 스크린샷 명령이 있을 때마다. 위험은 규칙 파일의 "Risk paths"와의 파일 대조이지 의견이 아니다. 팀은 올릴 수만 있고 걸린 것을 내리지 못한다. 의존성 매니페스트와 CI 설정은 출시 뒤에만 세는 둘째 목록에 있다. 출시 전의 scaffold나 버전 올림은 risk:low로 남는다.
- **머지는 저장소를 따른다.** 원격 없음 → 로컬 머지, 원격 있음 → PR. 어느 경우든 검증·리뷰·체크가 초록인 `risk:low` 작업은 lead가 머지한다 — 출시 전에는 `risk:high`도 그렇다(머지 메시지와 당신의 페이지 "기본값으로 정한 것"에 기록된다). 출시 뒤 `risk:high`는 당신의 한마디를 기다리며, PR의 리스크 요약을 보고 말한다. main은 항상 실행 가능하다. 상태판의 Run 줄이 제품을 켜는 법이다. 출시는 당신이 말한다. lead가 상태판의 Stage를 live로 한 번 바꾸고, 되돌아가지 않는다.
- **상태판이 당신의 창이다.** docs/STATUS.md는 당신의 페이지다 — Mode, 써 볼 것(이터레이션당 대본 하나), 당신이 할 일(최대 셋), 기본값으로 정한 것 — 훅이 상한을 강제하고 쉬운 말로 쓴다; docs/STATUS-team.md는 팀의 포인터(계획, 단계, 카운트)다. 둘 다 매 세션 시작에 주입되고 끊는 지점마다 커밋된다. 어긋나면 git이 진실이다.
- **세션은 끝나도 일은 안 끝난다.** lead는 매 세션 첫 메시지에서 상태판을 git과 대조하고, Mode가 running이면 스스로 이어간다. 끊김의 비용은 최대 진행 중이던 한 단계다. 컴팩션은 상태판을 다시 주입할 뿐이고, 세 번째에 lead가 단계를 마치고 상태판을 커밋하고 새 세션을 청한다.
- **실패는 환류된다.** 계획마다의 지표(재작업, 질문, 테스트, 소요)는 docs/METRICS.md로 가고, 이터레이션 끝마다 지표에 신호가 있으면 미니 회고가 스스로 돌아 번호 붙은 제안 최대 셋을 리뷰에 붙이고 당신은 번호로 승인한다; `/retro`는 청할 때 같은 것을 전부 돈다. 사고가 뒤에 있는 것만.
...
- 팀: 브레인스토밍(lead가 제안하고 반박한다; 어젠다 없음) 또는 가져온 문서의 정리 → "기획서 만들어줘" → docs/BRIEF.md 작성(채택 / 나중 / 기각과 이유; 흐름과 화면에 대한 당신 말은 부록에 그대로; 당신이 정하지 않은 슬롯은 기본값)과 critic 사전 부검을 한 번에 → 질문 한 라운드: 비목표, 확인할 기본값, 부검 결과 → 승인 → 문서 커밋과 같은 턴에 `/kickoff` — 헌장 도출(기본값은 기록), 스택 ADR(결정과 이유; 대안은 기획서가 스택을 비워 뒀을 때만; 디자인 기반은 한 줄, 보안 기준선은 골격에 들어가는 줄만), 질문 하나(스택, 예산, 그리고 돈이 들거나 보안에 닿는 미결 항목만), CLAUDE.md, 사양서 색인(절은 계획마다 그때 쓴다), 백로그, 골격과 최대 두 단계로 된 walking skeleton 계획, ADR·색인·계획을 한 번에 보는 critic 리뷰, 예산 답으로 채용 적용, 상태판은 `Mode: running`. 이터레이션 1 전에는 코드를 쓰지 않는다.
- 당신: 채팅에서 하듯 이야기하고, 기능에 대해 아는 걸 다 말한다 — 그대로 남아 사양서가 되고, 아무도 다시 묻지 않는다. 한 라운드의 질문에 솔직히 답한다. **구체적인 비목표 3개**는 당신만 줄 수 있는 것이고 이후 모든 판정의 기준이다. 성공 지표와 접는 기준은 기본값이 붙어 오니 확인하거나 바꾼다. 기획서를 승인한다. 승인이 곧 진행이다. 킥오프에서는 ADR의 이유(기획서가 스택을 비워 뒀다면 대안도)를 읽고 질문 하나에 답한다. 그다음 새 세션을 열고 "계속".
...
- 팀: 이터레이션마다 끝에 리뷰 — 써 볼 것(실행 명령과 3~5단계), 이번에 바뀐 것, 당신이 할 일(최대 셋: 아직 못 본 화면, 기본값이 붙은 결정, 당신 뜻인지 확신이 없는 것), 기본값으로 정한 것, 회고 제안, 다음 이터레이션. 리뷰 사이에도 당신 페이지의 "써 볼 것" 대본은 계획이 머지될 때마다 늘어난다.
- 당신: 제품을 써 본다. 계획·PR·사양서는 읽고 싶을 때만 읽는다. critic·reviewer·verifier가 읽었다. 그다음 원하는 것을 말한다 — "계속", 또는 지시.
- 조심할 것: "당신이 할 일" 줄 — 팀이 스스로 못 내리는 판단을 청하는 것이니 "계속" 전에 답한다. "기본값으로 정한 것" 중 아닌 것이 있으면 말한다 — 지시가 된다. 써 볼 것이 안 되는 것은 버그 보고다. 무엇을 했고 어떻게 됐는지 말한다.
...
- 팀: 한 문장에 확인이 자명하면(재현 있는 결함, 잘못된 문구·값·기본값) → `/hotfix`: 수정 전에 실패하는 회귀 테스트, 수정, 전체 검증, 리뷰 1렌즈(Risk path에 닿으면 2렌즈와 `risk:high` 라벨), 머지 — 이터레이션이 이어지기 전에. 나머지 → 다음 이터레이션 맨 앞에 `/plan bug: …`(지금이라 하면 지금): 조사 → 가설 → 실패하는 재현 테스트 → 수정 → 리뷰.
...
- 팀: docs/BACKLOG.md에 모든 항목이 완료 조건과 `spec:` 링크와 함께 있고, planner가 정리하며(기획서의 "나중" 줄과 사양서 절의 아직 안 덮인 완료 조건을 훑는다), 상태판의 "다음 이터레이션" 줄이 다음을 말한다. 끝난 항목과 출하된 계획의 내부 결정은 /release 때와 백로그가 300줄을 넘을 때 docs/archive/로 옮겨져 planner가 매번 읽는 파일이 짧게 유지된다. 지우는 것은 없다 — 보관된 항목은 이름을 대면 되살아난다.
...
- 팀: 한 번에 질문 하나, "결정 / 선택지 2~3개 / 이유 있는 추천 / 무응답 시 기본값" 형식. 당신 페이지의 "당신이 할 일"에 남으니 새 세션도 다시 묻는다. `risk:high` PR이면 리스크 요약 — Risk path 위의 파일과 바뀐 것, 깨질 수 있는 것, 렌즈마다 확인한 것, 부정 케이스 테스트, 롤백 — 과 두 가지 답, 머지 또는 보류. diff는 읽지 않는다. 렌즈 넷이 읽었다.
...
- 컴팩션: PreCompact 훅이 세고 SessionStart 훅이 그때마다 상태판을 다시 주입하니 lead는 이어 간다; 세 번째에 lead가 단계를 마치고 상태판을 커밋하고 새 세션을 청한다. 요약 위에 요약이 쌓이면 품질이 떨어지니 기본은 새 세션이다 — 한 세션 = 한 이터레이션. `claude --continue`는 짧게 끊긴 경우만.
...
- 팀: docs/METRICS.md 추세를 먼저 본다(재작업, 질문, 계획당 테스트, 소요; 머지된 slug를 가리키는 `bug:` 계획; 쌓이는 `hotfix:` 줄). git에서 착수 구간(기획서 승인 → 킥오프 완료 → 첫 계획 출하)도 본다 → 재작업·검증 실패·리뷰 blocker·당신의 개입이 난 지점을 3개 이내로 특정, 근본 원인 → 원인마다 재발 방지책과 그것이 박힐 자리(CLAUDE.md 한 줄 / 스킬 / 물리적으로 막는 훅 / "Risk paths"나 "UI paths" glob / 계획 형식 / 에이전트의 기억 파일) 제안 → 승인된 것만 반영. 이터레이션 끝마다 지표에 신호(두 번 개정된 계획, 상한을 넘긴 수정 루프, 반복된 verifier 실패, 머지된 작업의 버그)가 있으면 같은 것이 미니 모드로 스스로 돌아 번호 붙은 제안 최대 셋을 리뷰에 붙이고, "1, 3 반영하고 계속"에 반영된다.
- 당신: **위반 사례가 있는 규칙만** 승인한다. 일반론("항상 신중하게")은 거절한다. CLAUDE.md이 150줄이나 8KB를 넘기 시작하면 절차는 스킬로, 사실은 docs/ARCHITECTURE.md로 빼라고 한다.
- 조심할 것: 회고 없이 같은 프롬프트를 반복하기. 규칙을 잔뜩 승인해 CLAUDE.md을 비대하게 만들기.

### 3.12 병렬 작업 (고급)
- 언제: 파일 단위로 서로 독립적인 기능이 둘 이상 있고, 순차로는 시간이 아까울 때. **처음 2주는 쓰지 않는다.**
- 방식 1 (세션 병렬, 가장 견고): 터미널마다 `claude --worktree <기능명>`. 각 세션이 자기 브랜치에서 계획하고 돈다. 상태판은 메인 체크아웃 것이고 worktree 세션은 편집하지 않는다. 당신이 PR 순서대로 머지하고, 뒤 브랜치에는 rebase 후 재검증을 시킨다.
- 방식 2 (세션 내 병렬): `/parallel <계획 파일들>` → team-builder가 계획마다 격리 worktree에서 통째로 구현·자체 검증 → `/integrate`가 통합 브랜치에 하나씩 리뷰·머지 → `/ship`이 그 브랜치를 PR 하나로 출하. 계획 도중 끊긴 builder는 그 브랜치에서 마지막 커밋 다음 단계부터 다시 띄운다.
...
- [ ] 논리 단계마다 쉬운 말로 쓴 "증명" 줄이 하나에서 넷 있고(아니면 사유가 있는 `n/a`) 검증 명령이 있다. UI 단계는 디자인 기반이 들어간 뒤부터 네 가지 상태의 "shows" 줄이 있다(그 전에는 첫 UI 계획이 기반 단계로 시작한다). Risk path 계획은 위협 모델 줄과 진입점마다 부정 케이스 테스트가 있다
...
- 자동 주입 문서가 비대해짐: CHARTER 60줄, 당신의 페이지 1,800자(훅이 더 긴 쓰기를 거부한다), 팀 포인터 40줄, CLAUDE.md 8KB(하위 에이전트를 띄울 때마다 들어간다). CHARTER는 넘치면 planner가 분리하고, 훅은 주입 파일을 120줄에서 자른다.
- 계획은 질문을 끌지 않는다: 출처는 사양서 절·버그 줄·당신 말이고 planner의 기본값은 기록된다. `/plan`이 에스컬레이션 넷 밖의 것을 물으면 `/retro` 대상이다.
- 단계마다 verifier: 기본은 끔 — implementer의 빨강→초록 보고가 단계를 지고, verifier는 build 끝과 출하 전에 전체 검증으로 돌고, 리뷰 수정 라운드는 quick 검사만 돈다. 단계마다 독립된 빠른 검증을 원하면 운영 프로필(`/hire`)에서 켠다. risk:high와 레거시 계획은 어차피 돈다.
- 렌즈: 기본 둘, diff가 크거나 출시 뒤 위험이 높으면 넷, UI paths에 걸리고 스크린샷 명령이 있으면 ux 렌즈. 전부 기계적이고 전부 운영 프로필로 조정한다.
- 컴팩션: PreCompact 훅이 세고, 상태판이 다시 주입되어 lead가 이어 가며 세 번째 뒤에만 세션을 끝낸다 — 당신이 알아챌 필요가 없다.
