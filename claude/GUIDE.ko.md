...
그 외는 팀이 한다 — 사양서를 쓰고, 계획하고, 만들고, 리뷰하고, 저위험 작업은 스스로 머지하며, 계획 2~4개 이터레이션마다 멈춰 당신을 본다. 커맨드를 칠 필요는 없다. 말하면 lead가 절차를 돌린다. diff를 읽는 건 팀이 `risk:high` 작업(데이터·인터페이스·보안)을 머지해 달라고 할 때뿐이다.
...
당신이 받는 질문은 이것뿐이다. 킥오프 때 스택과 예산 한 번; 돈이 드는 것(유료 서비스, 새 런타임 의존성); 보안·인증·사용자 데이터에 닿는 것; 기획서의 비목표를 넘는 요청; 머지된 작업을 버리게 하는 변경; 팀이 세 라운드에 못 닫은 수정 루프; 머지할 `risk:high` 작업. 나머지는 팀이 기본값으로 정하고 docs/DECISIONS.md에 적으며, 거기서 읽을 수 있다.
...
첫 실행: 레포에서 `claude` → 폴더 신뢰 확인에 동의(훅이 켜진다) → settings의 `agent` 설정으로 team-lead가 메인 에이전트다 → 아이디어를 말한다(또는 직접 쓴 문서의 경로를 준다): lead가 함께 브레인스토밍하고 기획서를 정리해 승인을 청한다 → "진행해": 질문 하나(스택과 예산), 그다음 팀이 헌장·사양서·백로그·첫 계획을 쓰고 상태판을 `Mode: running`으로 남긴다 → 새 세션을 열고(모델 배정은 세션 시작 때 로드된다) "계속".
...
- **머지는 저장소를 따른다.** 원격 없음 → 로컬 머지, 원격 있음 → PR. 어느 경우든 검증·리뷰·체크가 초록인 `risk:low` 작업은 lead가 머지하고, `risk:high`는 당신을 기다린다. main은 항상 실행 가능하다. 상태판의 Run 줄이 제품을 켜는 법이다.
...
- 팀: 브레인스토밍(lead가 제안하고 반박한다; 어젠다 없음) 또는 가져온 문서의 정리 → "기획서 만들어줘" → docs/BRIEF.md 작성(채택 / 나중 / 기각과 이유; 흐름과 화면에 대한 당신 말은 부록에 그대로) → 브레인스토밍이 비워 둔 슬롯만 한 번 묻기 → critic 사전 부검을 교정 라운드로 당신에게 → 승인 → 문서 커밋. 그다음 "진행해": `/kickoff` — 헌장 도출(기본값은 기록), 디자인 기반과 보안 기준선을 담은 스택 ADR(critic 리뷰), 질문 하나(스택, 예산, 그리고 돈이 들거나 보안에 닿는 미결 항목만), 스모크 테스트 하나가 있는 골격, CLAUDE.md, 사양서, 백로그, walking skeleton 계획, critic의 사양서·계획 리뷰, 예산 답으로 채용 적용, 상태판은 `Mode: running`.
- 당신: 채팅에서 하듯 이야기하고, 기능에 대해 아는 걸 다 말한다 — 그대로 남아 사양서가 되고, 아무도 다시 묻지 않는다. 빈 슬롯에는 솔직히 답한다. **비목표 3개**, **측정 가능한 성공 지표**, **접는 기준**은 브레인스토밍이 빠뜨리는 것들이고 이후 모든 판정의 기준이다. 사전 부검 결과를 읽는다(항목마다 기본값이 붙어 있다). 기획서를 승인한다. 킥오프에서는 ADR의 "왜 다른 대안이 아닌가"를 읽고 질문 하나에 답한다. 그다음 새 세션을 열고 "계속".
- 조심할 것: 숫자나 관찰 가능한 사실이 아닌 성공 지표. 무엇을 왜 기각했는지 없는 기획서. 스택 결정을 팀에 통째로 맡기는 것 — 되돌리기 가장 비싼 결정이다. 나중에 방향이 바뀌면 말하면 된다. lead가 기획서를 개정하고 헌장과 사양서가 따라온다.

### 3.2 레거시 인수
- 언제: 기존 코드가 있고 고치거나 갈아엎어야 할 때.
- 당신이 말하는 것: 왜 리빌드하나, 목표 상태, 바뀌면 안 되는 것, 기한 — 한 문단과 "기획서 만들어줘"면 충분하다. 그다음 "진행해", lead가 물으면 경로.
- 팀: 짧은 리빌드 기획서(Kind: 레거시) → `/assess`: 인벤토리(docs/ASSESSMENT.md; 기획서와 충돌하는 "버그로 보이는 동작"은 표시된다) → 보존/수정 방침을 한 번에 → 전략과 첫 seam(ADR, docs/REBUILD_PLAN.md, CLAUDE.md), 헌장은 도출해 기록 → 질문 하나(전략과 예산) → 첫 seam의 parity harness 계획(critic 리뷰) → 문서 커밋 → 채용 → 상태판은 `Mode: running`. 이후 리빌드 단계마다 그 단계의 seam까지 하네스를 먼저 확장한다.
...
### 3.7 팀이 물을 때
- 언제: 에스컬레이션 — 0절의 목록 — 또는 `risk:high` PR.
- 팀: 한 번에 질문 하나, "결정 / 선택지 2~3개 / 이유 있는 추천 / 무응답 시 기본값" 형식. 상태판의 CEO 대기 줄에 남으니 새 세션도 다시 묻는다. `risk:high` PR이면 검증 증거, 리뷰 지적과 처리, 리스크와 롤백, 그리고 diff.
...
- main 머지는 저장소 상태를 따른다(`/ship`이 판정하고 `/policy`가 판정과 근거를 보여 준다):
  - 원격 없음 → `local`: PR 설명이 docs/prs/NNNN-<slug>.md로 남고 lead가 `merge --no-ff`로 로컬 머지한다(`risk:high`는 당신 승인 뒤).
  - 원격 있음, 보호 없음 → `manual`: 팀이 push하고 PR을 열고 `risk:low`는 체크 통과 후 머지한다. `risk:high`는 당신을 기다린다.
...
- 팀: docs/METRICS.md 추세를 먼저 본다(재작업, 질문, 계획당 테스트, 소요; 머지된 slug를 가리키는 `bug:` 계획; 쌓이는 `hotfix:` 줄) → 재작업·검증 실패·리뷰 blocker·당신의 개입이 난 지점을 3개 이내로 특정, 근본 원인 → 원인마다 재발 방지책과 그것이 박힐 자리(CLAUDE.md 한 줄 / 스킬 / 물리적으로 막는 훅 / "Risk paths"나 "UI paths" glob / 계획 형식 / 에이전트 memory) 제안 → 승인된 것만 반영.
...
- `risk:high` PR을 diff와 증거를 읽지 않고 초록불만 보고 머지하기
