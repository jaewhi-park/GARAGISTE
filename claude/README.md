...
| Engineering | `/plan <item>` → `/run <plan>` (or `/build`) | one intake call → docs/plans/*.md, one commit per step |
...
- The human's four jobs: answer charter and intake questions / approve plans and decide escalations / merge PRs (per policy) / approve retro rules.
