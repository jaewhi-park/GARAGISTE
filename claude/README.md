...
|---|---|---|
| Governance | `/brainstorm <idea or file>` → `/kickoff` / `/assess <target>` | docs/BRIEF.md (the product brief: brainstorm or your document → one round of non-goals, defaults and the critic's pre-mortem → approval), then docs/CHARTER.md, docs/adr/, docs/SPEC.md (the index; the docs/specs/ files come with /plan), docs/BACKLOG.md, CLAUDE.md, (legacy) docs/ASSESSMENT.md · docs/REBUILD_PLAN.md · parity-harness plan; /hire in the closing step, then the board |
| Running mode | (no command — say "계속") / `deliver` | one iteration: plans through plan → run → merge up to the unattended cap (default 8 plans or 2 spec sections), then an iteration review; docs/STATUS.md (the CEO's page: Mode, Try it, For you, Decided by default) and docs/STATUS-team.md (the team's pointer) carry the state; `instruction` (a knowledge skill) is how the lead turns what the CEO says into the right procedure |
...
| Hotfix | `/hotfix <what and why>` | one pass on hotfix/<slug>, no plan file: implement + regression test → full verification → one correctness lens → PR (never auto-merged) or local merge; over 3 files / 50 logic lines it becomes a /plan; on a Risk path it adds the security lens and the risk:high label; one docs/METRICS.md line marked `hotfix:` |
...
| Handoff | `/handoff` (mid-flight only) / `/resume` (the lead runs it itself at session start) | docs/STATUS.md and docs/STATUS-team.md (committed at the cut points) |
...
|---|---|---|---|
| team-lead | tech lead / EM; main-session agent | Agent(team-*, Explore), Read/Grep/Glob, Bash, Edit/Write, AskUserQuestion | Edit/Write on the board files (docs/STATUS.md, docs/STATUS-team.md) only; shell read-only plus git branch/merge/sync commands, docs-only `git add`/`commit` (docs/, CLAUDE.md, .claude/, .gitignore, CHANGELOG), branch pushes, gh PR commands and the scripts; never runs code, never writes through the shell, never commits a code path — all hook-enforced; default via `settings.agent` |
| team-planner | architect; docs, plans, specs, ADRs | Read/Grep/Glob, Edit/Write, Bash, Web* | memory in docs/memory/team-planner.md — accumulates architecture knowledge; shell read-only (hook) |
...
| team-reviewer | per-lens code review (correctness · security · performance · maintainability · ux) | Read/Grep/Glob, Bash, Edit/Write | memory in docs/memory/team-reviewer.md — the one file it may write (hook); accumulates recurring defect patterns; shell read-only plus the toolchain to run a test (hook) |
...
- Only the lead asks the CEO, and only for the four things that are expensive to reverse (money, security or user data, a non-goal conflict, discarding shipped work; a risk:high merge once live). Format: one-sentence decision / options / recommendation / default if unanswered — except /brainstorm and the revision discussions, where open questions and free-text answers are allowed.
- Fix loops are capped at 3 rounds; past that, the loop rule cuts the plan after its last passed step and the rest becomes a `rework:` item — never a question.
- Autonomous decisions go on the CEO's page (Decided by default) for the iteration and, when hard to reverse, to docs/DECISIONS.md; architecture decisions to docs/adr/; technical debt to docs/DEBT.md.
...
## Session lifecycle
The session is working memory; the repo is long-term memory. State lives in three places — one commit per step, the plan file (committed at approval), the board — `docs/STATUS.md`, the CEO's page, and `docs/STATUS-team.md`, the team's pointer (auto-injected by the SessionStart hook, committed at the cut points — the approval commits, ship, a question waiting on the CEO, handoff, a compaction stop — and changed uncommitted between them; the repository is the truth and the board the pointer). One session = one plan (PR). A finished /ship or /plan leaves board and commits in place; when a board exists the lead runs `resume` itself at the first turn (board against git, worktrees and PR state — the repository wins), so the CEO just talks; `/handoff` only when stopping mid-flight. Interruptions (Esc, an error, a usage cap) cost at most the step in progress: step commits are the checkpoints, /build continues an interrupted step from the uncommitted changes and /parallel relaunches an interrupted builder on its branch. Compactions are counted by the PreCompact hook (`.claude/session/compactions`) and the SessionStart hook re-injects the board after each one, so the lead continues; at the third it finishes the step, commits the board and ends the session. Use `claude --continue` only for short interruptions. Planner and reviewer accumulate long-term knowledge in `docs/memory/<name>.md` — repository files, so both flavors share the mechanism and the CEO can read them.

## Differences from the opencode flavor
- Role rules are enforced by `guardrails.mjs` from the hook's `agent_type`, mirroring the opencode permission blocks but as deny-lists of what changes state rather than allow-lists of tool names (a new stack needs no new pattern): team-lead edits the two board files only (a write that would leave docs/STATUS.md over 1,800 characters is refused), its shell is read-only plus git branch/merge/sync, docs-only `git add`/`commit` (the hook refuses a code path), branch pushes and gh PR commands — no code runs, no shell writes; team-planner (and Explore) edit only docs/** and CLAUDE.md and their shell is read-only (git status/diff/log/show, listings, version and dependency checks); team-reviewer's shell is read-only plus the toolchain, so it can run a test to check a claim, and its only writable file is docs/memory/team-reviewer.md; team-builder may push its branch but cannot merge, rebase, pull or touch worktrees; team-verifier runs the toolchain plus whatever CLAUDE.md's "## Commands" lists — an allow-list, because it is CI. team-implementer and team-critic are scoped by their tools line and the generic rules; a role created by /recruit gets the generic rules only — add an `agent_type` branch to the hook for a hard boundary.
...
- Wrapper commands (`time`, `env`, `nice`, `timeout`, `sh -c`) are unwrapped before a role's shell rule is applied, so a read-only role cannot run code behind them.
