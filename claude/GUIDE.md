...
First run: run `claude` in the repo → accept the folder-trust prompt (this enables hooks) → `settings.agent` makes team-lead the main agent → `/kickoff` for a new project, `/assess` for legacy, `/resume` to continue.
Check: right after SessionStart you see "GARAGISTE: docs/CHARTER.md is missing…" (hook works), and `/kickoff` etc. appear in the slash menu while knowledge skills such as `charter` do not.
...
### Language
Agent prompts are English. Responses, questions and documents follow the `## Language` line in CLAUDE.md (asked during `/kickoff`); when unset, agents mirror the language you write in.
...
- Command: none. During `/plan` the planner produces a comparison table and an ADR draft; the critic adds a pre-mortem; the lead asks when escalation criteria are hit.
...
- Team: `wip:` commit of uncommitted changes → docs/STATUS.md updated → report "finished / waiting / first action next session".
...
- Team: reconciles the status board (auto-injected), the plan and git → trusts the repo and fixes the board on disagreement → verifier first if the last result was FAIL → asks about waiting decisions → continues from the next action.
...
## 6. Do not
- Order large work in chat without a plan
