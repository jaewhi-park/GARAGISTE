...
The team does the rest — it writes the spec, plans, builds, reviews and merges low-risk work on its own, plan after plan up to the unattended cap (default 8 plans or 2 spec sections) — and stops to look at you there; a new screen, a first run or a decision taken by default is a line on your page, not a stop. You never have to type a command: say it, and the lead runs the procedure. You never read a diff: once the product is live, `risk:high` work (data, interfaces, security) waits for your merge or hold, said from the PR's Risk summary.
...
| "계속" | Runs plan after plan (plan → build → review → ship → merge) up to the unattended cap, then stops with an iteration review | Read the review, try what it lists, answer "써봤다 · what worked" or "안 써봤다", say what you want — this session or a new one |
...
First run: run `claude` in the repo → accept the folder-trust prompt (this enables hooks) → `settings.agent` makes team-lead the main agent → say your idea (or give the path of a document you wrote): the lead brainstorms with you, compiles the brief, asks you one round (three non-goals, the defaults it set, the critic's findings) and then to approve it → approval starts the kickoff: one question (the stack, the budget and, when the product calls a model, the reference model), then the team writes the charter, the stack ADR, the rules file, the spec index, the backlog and the first plan (the walking skeleton: the scaffold and at most two steps), and leaves the board at `Mode: running` → open a new session (model assignments load at session start) and say "계속".
...
When `/kickoff` or `/assess` finishes, the lead runs `/hire` with the budget tier you gave there and the project's character from the brief: it confirms available models (aliases opus/sonnet/haiku), proposes a role × model table with reasons and applies it — no second question. The roster is shown; change a line by saying so. Run `/hire` yourself when the budget changes (then it asks the tier). Principle: the strongest model where judgment happens (planner, critic, reviewer), the fastest model for the verifier, the implementer by budget. A script changes only the `model:` lines and an "Operating profile" is left in CLAUDE.md; it takes effect from the next session. `/roster` shows the current assignment.

### Language
Agent prompts are English. Responses, questions and documents follow the `## Language` line in CLAUDE.md. The lead asks for it once at the first brainstorm; `/lang <code>` changes it any time (a script rewrites only that section). When unset, agents mirror the language of your latest message, but an English command template can still tip the lead into English: if that happens, say so or run `/lang`.
...
- **Work runs in iterations.** The first is the walking skeleton alone, so you see it before anything is built on it; after that the lead takes backlog items in order, up to the unattended cap (default 8 plans or 2 spec sections; /hire sets it), and for each: a plan from the spec section (the critic reviews every plan; its APPROVE is the approval), then build → review → ship → merge. At the cap it writes the iteration review and stops; a new screen, a first run or a decision taken by default is a line on your page along the way, never a stop. Say 계속 in this session or a new one — the board carries everything. Once the product is live, a `risk:high` PR pauses the iteration until you merge it, because the next plan may build on it.
...
- **The board is your window.** docs/STATUS.md is your page — Mode, Try it (one script for the iteration), For you (at most three lines) and Decided by default — kept under a size the hook enforces, in plain words; docs/STATUS-team.md is the team's pointer (the plan, the step, the counts). Both are injected at every session start and committed at each cut point. Git is the truth when they disagree.
- **Sessions end; the work does not.** The lead reconciles the board with git at the first message of every session and continues on its own when Mode is running. An interruption costs at most the step in progress. A compaction only re-injects the board; a third one makes the lead finish the step, commit the board and ask for a new session.
...
- Compaction: the PreCompact hook counts it and the SessionStart hook re-injects the board after each one, so the lead continues; at the third the lead finishes the step, commits the board and asks for a new session. Summaries stacked on summaries degrade the work, so the default is a fresh session — one session = one iteration. Use `claude --continue` only for a short interruption.
...
- Option 1 (session-level, most robust): one terminal per feature with `claude --worktree <feature>`; each session plans and runs on its own branch; the board belongs to the main checkout and the worktree session never edits it. You merge the PRs in order and ask later branches to rebase and re-verify.
- Option 2 (in-session): `/parallel <plan files>` → one team-builder per plan in its own worktree, implementing and self-verifying the whole plan → `/integrate` reviews and merges them one at a time into an integration branch → `/ship` ships that branch as one PR. A builder interrupted mid-plan is relaunched on its branch from the step after its last commit.
- Prerequisites: `.claude/` committed (or listed in `.worktreeinclude`) so worktree sessions have the team; `.env` in `.worktreeinclude` too.
...
- Auto-injected documents growing: CHARTER 60 lines, your page 1,800 characters (the hook refuses a longer write), the team's pointer 40 lines, CLAUDE.md 8 KB (it goes into every subagent spawn). The planner splits CHARTER when exceeded; the hook truncates injected files at 120 lines.
...
- Compaction: the PreCompact hook counts it; the board is re-injected and the lead continues, ending the session only after the third — you never have to notice it.
...
- Leave the verification commands in CLAUDE.md out of sync with reality
