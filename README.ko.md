...
파란 선이 매 작업이 밟는 기본 경로다: `/backlog` → `/plan` → `/build` → `/review` → `/ship` → 머지. `/run` 은 가운데 세 역을 한 번에 지난다. 빨간 선은 `/hotfix`, 한 문장으로 말할 수 있는 수정을 계획 없이 한 번에 처리하는 길이다. 초록 선은 병렬 작업: `/parallel`(Claude Code) 또는 `/spawn`(opencode) 뒤 `/integrate` 를 거쳐 `/ship` 으로. 보라 선은 머지 뒤의 일: 주 1회 `/release`, 실패가 반복되면 `/retro`, 그다음 다음 `/plan`. 되돌아가는 선 위의 이터레이션 리뷰 역은 계획 2~4개마다 당신이 제품을 써 보고 \"계속\"이나 지시를 말하는 곳이다. 겹친 원이 당신이 답하거나 승인하는 역이고, 나머지 역은 팀이 알아서 지난다.
