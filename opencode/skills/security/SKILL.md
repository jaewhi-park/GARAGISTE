...
- /review: the security lens is always on (2-lens default) and works through the checklist above; a Risk-paths hit makes the review 4 lenses once the board's Stage is live; pre-launch the two default lenses stay and the threat-model lines carry the check. A hotfix that hits a Risk path keeps the hotfix path and gets this lens beside correctness (the `hotfix` command).
