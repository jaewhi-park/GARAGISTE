...
3. Record plan path · slug · branch · path under the "Parallel in progress" list on the board (docs/STATUS-team.md), and set the Now block to `Plan: parallel — <n> plans · Branch: main · Shipped: —`, `Step: none`, Next actions: 1. `/integrate` (after the worktree sessions finish) — so a resumed session never re-runs a plan a worktree session is building.
