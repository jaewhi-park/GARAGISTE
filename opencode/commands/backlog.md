...
1. Have team-planner update docs/BACKLOG.md. Item format: title / one-sentence user value / completion criteria (verifiable) / size (S·M·L) / priority (P0–P2) / dependencies / spec (docs/specs/NNNN-<slug>.md or —) / req (the requirement ID the item came from, or —). Also sweep docs/BRIEF.md (its Later lines and appendix) and specs with Status approved, in progress or done for candidates: their "Not this time" items and any "Done when" item no shipped plan covers.
...
5. Commit docs/BACKLOG.md yourself (`docs(backlog): groom <date>`) on the current branch, and remove a `Brief: … revised` line from the board yourself (the revision is groomed in).
