...
1. Branch: read the plan file; if the current branch is main/master, run `git switch -c plan/<slug>` yourself.
2. Implement (= /build): per step, in order, assign it to team-implementer with plan path, step number and the step's lines, then quick verification with team-verifier (skipped only when the operating profile says `per-step verifier: off` and the implementer's report shows every quick command at exit 0). On FAIL, pass the report verbatim for a fix (max 3). On PASS, update the status board and continue. After the last step, full verification once more and record `PASS full @<hash>` in STATUS.
