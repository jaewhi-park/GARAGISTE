...
The board's `Mode:` line is the contract with the CEO: `running` from the first step here to the pause at the end, `waiting on CEO` while a question is open, `paused — …` when the team is looking at the CEO and doing nothing else. Every step is re-entrant (the lead's rule): a session that opens with `Mode: running` re-enters here after `resume` and continues with the plan and step the board names — the CEO never has to repeat the go.
...
1. For each item, in order:
   a. a compaction summary in your context is the compaction cut (the lead's rule): it applies before a new plan starts.
   b. Do the /plan procedure (read .opencode/commands/plan.md) for the item. The critic's APPROVE is the approval. An escalation question that gets no answer in this turn ends the turn with `Mode: waiting on CEO` (the Waiting-on-CEO line carries the default); the CEO's next message resumes here.
