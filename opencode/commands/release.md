...
1. In parallel, have team-planner compare commits since the last tag with the CHANGELOG and propose a semver version (ask the CEO if interfaces changed) — the same call applies the `backlog` command's Archive rule (read .opencode/commands/backlog.md) to docs/BACKLOG.md and docs/DECISIONS.md and reports the counts — and run full verification with team-verifier. On verifier FAIL, do not release.
