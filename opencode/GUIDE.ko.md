...
- 팀: docs/METRICS.md 추세를 먼저 본다(재작업, 질문, 계획당 테스트, 소요; 머지된 slug를 가리키는 `bug:` 계획; 쌓이는 `hotfix:` 줄). git에서 착수 구간(기획서 승인 → 킥오프 완료 → 첫 계획 출하)도 본다 → 재작업·검증 실패·리뷰 blocker·당신의 개입이 난 지점을 3개 이내로 특정, 근본 원인 → 원인마다 재발 방지책과 그것이 박힐 자리(AGENTS.md 한 줄 / 스킬 / 물리적으로 막는 훅 / "Risk paths"나 "UI paths" glob / 계획 형식 / 에이전트 memory) 제안 → 승인된 것만 반영.
