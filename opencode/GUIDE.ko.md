...
첫 실행: 레포에서 `opencode` → Tab 으로 `team-lead` 선택 → 아이디어를 말한다(또는 직접 쓴 문서의 경로를 준다): lead가 함께 브레인스토밍하고 기획서를 정리해 승인을 청한다 → "진행해": 질문 하나(스택과 예산), 그다음 팀이 헌장·사양서·백로그·첫 계획을 쓰고 상태판을 `Mode: running`으로 남긴다 → 새 세션을 열고(모델 배정은 세션 시작 때 로드된다) "계속".
...
- 팀: 짧은 리빌드 기획서(Kind: 레거시) → `/assess`: 인벤토리(docs/ASSESSMENT.md; 기획서와 충돌하는 "버그로 보이는 동작"은 표시된다) → 보존/수정 방침을 한 번에 → 전략과 첫 seam(ADR, docs/REBUILD_PLAN.md, AGENTS.md), 헌장은 도출해 기록 → 질문 하나(전략과 예산) → 첫 seam의 parity harness 계획(critic 리뷰) → 문서 커밋 → 채용 → 상태판은 `Mode: running`. 이후 리빌드 단계마다 그 단계의 seam까지 하네스를 먼저 확장한다.
