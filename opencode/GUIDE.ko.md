...
### 모델·예산 — `/hire`
`/kickoff` 또는 `/assess` 가 끝나면 lead 가 거기서 답한 예산 티어와 기획서의 프로젝트 성격으로 `/hire` 를 실행한다. 사용 가능한 모델(`opencode models`)을 확인하고 역할 × 모델 표를 이유와 함께 만들어 바로 적용한다 — 두 번 묻지 않는다. 배정표가 보이고, 한 줄 바꾸려면 말하면 된다. 예산이 바뀌면 `/hire` 를 직접 실행한다(그때는 티어를 묻는다). 원칙: 판단하는 자리(planner·critic·reviewer)에 강한 모델, verifier 는 가장 빠른 모델, implementer 는 예산에 따라. 스크립트가 `model:` 줄만 바꾸고 AGENTS.md 에 "운영 프로필"이 남으며, 새 세션부터 반영된다. 현재 배정은 `/roster` 로 본다.
...
- **상태판이 당신의 창이다.** docs/STATUS.md는 당신의 페이지다 — Mode, 써 볼 것(이터레이션당 대본 하나), 당신이 할 일(최대 셋), 기본값으로 정한 것 — guardrail이 상한을 강제하고 쉬운 말로 쓴다; docs/STATUS-team.md는 팀의 포인터(계획, 단계, 카운트)다. 둘 다 매 세션 시작에 주입되고 끊는 지점마다 커밋된다. 어긋나면 git이 진실이다.
- **세션은 끝나도 일은 안 끝난다.** lead는 매 세션 첫 메시지에서 상태판을 git과 대조하고, Mode가 running이면 스스로 이어간다. 끊김의 비용은 최대 진행 중이던 한 단계다. 컴팩션 요약은 상태판 항목을 다시 실을 뿐이고, 세 번째에 lead가 단계를 마치고 상태판을 커밋하고 새 세션을 청한다.
...
- 컴팩션: 컴팩션 플러그인이 요약에 표시와 번호를 남기고 lead는 두 번째까지 이어 간다; 세 번째에 lead가 단계를 마치고 상태판을 커밋하고 새 세션을 청한다. 요약 위에 요약이 쌓이면 품질이 떨어지니 기본은 새 세션이다 — 한 세션 = 한 이터레이션. `opencode -c`는 짧게 끊긴 경우만.
...
- 자동 주입 문서가 비대해짐: CHARTER 60줄, 당신의 페이지 1,800자(guardrail이 더 긴 쓰기를 거부한다), 팀 포인터 40줄, AGENTS.md 8KB(하위 에이전트를 띄울 때마다 들어간다). CHARTER는 넘치면 planner가 분리한다 — 나머지 파일에는 opencode에 강제 상한이 없어서 이 선을 넘긴 파일은 그대로 매 세션 전체가 주입된다.
...
- 컴팩션: 컴팩션 플러그인이 요약에 표시와 번호를 남기고, lead는 이어 가며 세 번째 뒤에만 세션을 끝낸다 — 당신이 알아챌 필요가 없다.
