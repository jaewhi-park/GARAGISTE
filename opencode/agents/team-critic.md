...
- Spec-backed plans (the Source line names a docs/specs/ file): does the plan widen the spec's Done-when or pull in a Not-this-time item (blocker)? If it covers only part of the spec, does section 1 say "part k of n" and are the other parts in BACKLOG (blocker if not)? Is every spec "undecided" item in section 6 with a default or explicitly out of scope? Does it contradict docs/PARITY.md without a DECISIONS.md entry?
