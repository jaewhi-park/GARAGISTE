...
- An experiment step (kind: experiment): are its `bench:` command, N, threshold and verdict rule written before the run, and is every outcome's branch pre-written? A miss that would revert merged work, revise the plan or ask the CEO is a blocker. Length: a plan over 120 lines is minor unless the excess is proves, shows or threat-model lines.
