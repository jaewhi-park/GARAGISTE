---
description: Runs the verification commands from AGENTS.md (tests, lint, typecheck, build) and reports PASS/FAIL only. Never fixes code. Used at the end of /build and before shipping (a review fix round runs the implementer's quick verification only); per step only when the operating profile or the plan's risk says so.
mode: subagent
temperature: 0
steps: 20
permission:
