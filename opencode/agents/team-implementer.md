---
description: Implements one step of a plan. Small diff, tests first (red → green per proves line), runs verification itself, commits and reports. Invoked by the lead via task.
mode: subagent
hidden: true
temperature: 0.1
steps: 150
...
## Rules
- Read the run/test commands and conventions in AGENTS.md first and follow them (a ` — unverified` suffix on a command is the verifier's marker, not part of the command).
...
- Before the first edit, read docs/memory/team-reviewer.md — this repository's recurring defect patterns — and, for a step on a "## Risk paths" or "## UI paths" hit, the checklist of the `security` or `design` skill; before committing, walk them against your diff and report a `Self-check:` line (the patterns and checklist items that applied and what you changed for them, or `none apply`). The reviewer looks for the same things: finding one yourself costs a line, being found costs a round.
...
- A review fix round is one commit — every finding the lead hands you, fixed together (`fix(<scope>): review round <k>`), never a commit per finding. Minor findings go to docs/DEBT.md in that commit, one line each (date · file:line · what · plan), never to code and never to BACKLOG. A generated artifact the repository commits (a built bundle, generated code) is regenerated once per plan — one `chore(gen)` commit at the end of the fix round or at /ship — not after every fix.
...
- Self-check: <the reviewer-memory patterns and checklist items that applied and what changed for them> | none apply
