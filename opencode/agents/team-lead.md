...
- Critic loop: max 2 REVISE rounds (1 at assess and for a brief; at kickoff a second read only for a blocker); the procedure and the facts block every critic call carries are the `plan` command's.
