...
- Keep docs/CHARTER.md under 60 lines (the guardrail refuses more) and AGENTS.md under 8 KB — both are injected into every session and every spawn. Move detail elsewhere and link it.
- A plan is at most 12 KB — the guardrail refuses a write over it — and about 120 lines: sections 3 and 5 are a few lines each (the files that matter; each risk with its mitigation, the rollback in one line), section 6 only when it has an item; proves, shows and threat-model lines are never cut to fit — a plan that does not fit is cut by tryable outcome (the plan-size rule), never by shrinking those lines.
...
Steps line: `Steps: <n> logic + <m> mechanical [+ <k> experiment] [— over the plan-size target: <reason>]` — the lead and the critic read the critic threshold from it; `Small: yes` after it when the plan has at most two logic steps, at most 100 changed logic lines estimated, no Risk-path hit and no design-foundation step (the `plan` command's small mode)
