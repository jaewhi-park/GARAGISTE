...
| (no command — say "계속") / `/deliver` | One iteration: plans through plan → run → merge up to the unattended cap (default 8 plans or 2 spec sections), then an iteration review; docs/STATUS.md (the CEO's page: Mode, Try it, For you, Decided by default) and docs/STATUS-team.md (the team's pointer) carry the state. The lead turns what the CEO says into the right procedure per the `instruction` skill |
...
| `/hotfix <what and why>` | Small, well-specified change without a plan: implement + regression test → full verification → one correctness lens → PR (never auto-merged) or local merge on hotfix/<slug>; over 3 files / 50 logic lines it becomes a /plan; on a Risk path it adds the security lens and the risk:high label; one docs/METRICS.md line marked `hotfix:` |
...
## Session lifecycle
The session is working memory; the repo is long-term memory. State lives in three places — one commit per step, the plan file (committed at approval), the board — `docs/STATUS.md`, the CEO's page, and `docs/STATUS-team.md`, the team's pointer (auto-injected every session, committed at the cut points: the approval commits, ship, a question waiting on the CEO, handoff, a compaction stop; changed uncommitted between them, and the repository is the truth). When a board exists the lead reconciles it at the first turn, so the CEO just talks; an interruption costs at most the step in progress (step commits are the checkpoints). A compaction summary in the lead's context is numbered by the plugin: the lead continues after the first two and, at the third, finishes the step, commits the board and ends the session. Planner and reviewer accumulate long-term knowledge in `docs/memory/<name>.md` — repository files, so both flavors share the mechanism and the CEO can read them.
...
- Mid-session compaction: `plugins/compaction.ts` forces the board's items into the summary and numbers the compaction; `plugins/guardrails.ts` refuses a write that would leave docs/STATUS.md over 1,800 characters.
